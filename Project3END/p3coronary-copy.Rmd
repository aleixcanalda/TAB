...
coronary.plink <-  read.plink(bed = "data/coronary/coronary.bed",
                             bim = "data/coronary/coronary.bim",
                             fam = "data/coronary/coronary.fam")
```
...
```{r obesity}
coronary.phenotype <- read.delim("data/coronary/coronary.txt")
...
use <- info.snps$Call.rate > 0.95 &
       info.snps$MAF > 0.05 &
       abs(info.snps$z.HWE < 3.3) # This is only possible if we have controls, otherwise, we use info.snps    
...
```{r transform-data}
snpgdsBED2GDS("data/coronary/coronary.bed",
              "data/coronary/coronary.fam",
              "data/coronary/coronary.bim",
              out="coronaryGDS"
              )
```
...
#gwas.adj <- snp.rhs.tests(ev3 ~ smoke,  data = phenotype.qc,
                         #snp.data=genotype.qc, family = "Gaussian")

# Example with a quantitative trait (age)
gwas <- snp.rhs.tests(age ~ 1,  data = phenotype.qc,
                         snp.data=genotype.qc, family = "Gaussian")
...
manhattanPlot <- manhattanPlot + labs(x = 'Chromosome', y = 'P-value')
manhattanPlot
...
manhattanPlot <- manhattanPlot +
geom_hline(yintercept = -log10(1e-05))
manhattanPlot
...
##### &emsp;<i class="fas fa-comment-dots"></i> Answer: Són el rs7349741, rs12201995, rs6931936 i rs1935960
...
write.table(gwasStats[gwasStats$P<significance,], file="gwasStatspvalue.txt", quote = FALSE, row.names = FALSE, sep = "\t")
```
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer: Tant el rs6931936 com el rs1935960 estan en la zona codificant mentre que el rs7349741 i el rs12201995 no estan dins de cap gen.
