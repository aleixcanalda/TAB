...
```{r answer1.1}
"Using alpha for a discrete variable is not advised" because you can't visualize it correctly.
```
</div>
...
#It is joining both columns, creating a single one taking the data into a percentage.
```
...
class(mtcars$gear)
#it's numeric so we have to change it into a factor
mtcars$gear<-as.factor(mtcars$gear)
ggplot(data = mtcars, mapping = aes(x = cyl_f, fill = gear)) +
  geom_bar(position = "fill")
```
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(. ~ cut)
#the graphs are placed in a vertical position.
```
...
<div style="background-color:#F0F0F0">
##### <i class="fas fa-comment-dots"></i> Answer:
La millor seria la 1a opcio ja que pots comparar mitges i les desviacions millor.
</div>
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  ...
  facet_grid(cut ~ .) +
  scale_fill_grey()
#I'd use viridis_d because it goes from dark to light colors.
```
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) +
  labs(subtitle = "Yeah", caption = "Aha", tag = "Woah")
#To put titles and other texts on the graph.
```
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  ...
  facet_grid(cut ~ .) +
theme_test()
#the one that maximizes the data-ink ratio is theme_linedraw()
...
#when you zoom an svg file it doesn't pixelate, while a png file does, because since we're working with vectors with svg you can even move the dots and modify them.
```
...
p <- ggplot(data= economics, mapping = aes(x = date, y = unemploy)) + 
  geom_line() + 
  ggtitle("Evolution of unemployment") + 
...
```{r readSorted}
readDepth <- read.table("sorted.depth.txt", sep='\t', header=FALSE)
...
```{r ST}
st <- read.table("https://bioinformatica.uab.cat/base/documents/sgbcursos/documents/ST2019_11_5D12_11_13.txt", sep='\t', header=FALSE)
...
```{r pi, eval = F}
pi <- read.table("https://bioinformatica.uab.cat/base/documents/sgbcursos/documents/pi2019_11_5D12_35_42.txt", sep='\t', header=TRUE)
